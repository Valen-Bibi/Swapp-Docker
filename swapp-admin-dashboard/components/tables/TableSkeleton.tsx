		...
		<div className="p-6 relative">
			...
			<div className="overflow-hidden rounded-xl border border-swapp-tiza-verdoso dark:border-swapp-azul-petroleo bg-swapp-blanco dark:bg-swapp-azul-oscuro shadow-sm transition-colors">
				<table className="w-full text-left text-sm text-swapp-azul-petroleo dark:text-swapp-tiza-verdoso">
					<thead className="bg-swapp-tiza-verdoso/30 dark:bg-swapp-azul-petroleo/20 border-b border-swapp-tiza-verdoso dark:border-swapp-azul-petroleo">
						<tr>
							{[...Array(6)].map((_, i) => (
								<th key={i} className="px-6 py-4">
									<div className="h-4 w-24 rounded bg-swapp-tiza-verdoso dark:bg-swapp-azul-petroleo animate-pulse"></div>
								</th>
							))}
						</tr>
					</thead>
					<tbody className="divide-y divide-swapp-tiza-verdoso/50 dark:divide-swapp-azul-petroleo/50">
						{[...Array(6)].map((_, rowIndex) => (
							<tr key={rowIndex}>
								{[...Array(6)].map((_, colIndex) => (
									<td key={colIndex} className="px-6 py-4">
										<div className="h-4 w-full max-w-[120px] rounded bg-swapp-tiza-verdoso/50 dark:bg-swapp-azul-petroleo/50 animate-pulse"></div>
									</td>
								))}
							</tr>
						))}
